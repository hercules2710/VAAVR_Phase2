...
public class CylinderMove : MonoBehaviour
{
    ...
    public Task liftWheelTask;
    public float speed; 

    private bool isContact;
    private bool isPlaced;
    private bool isFinishedLifted = false;
    ...
    void Update()
    {
        ...
        {
             if (!liftWheelTask.isFinished && !isFinishedLifted)
            {
                 MoveUp();
            }
            else
            {
                isFinishedLifted = true;
            }
        }
    ...
    private void liftObj() // lift the object without rigibody
    {
        if (objHit == null)
        {
            cylinder1.GetComponent<FreezeJackMovement>().UnFreezeJack();
            return;
        }

        if (objHit.layer == LayerMask.NameToLayer("CanBeLifted"))
        {
            objHit.transform.position = objHit.transform.position + new Vector3(0f, speed * Time.deltaTime, 0f);
            if(speed > 0) cylinder1.GetComponent<FreezeJackMovement>().FreezeJack();
        }
    }
}
